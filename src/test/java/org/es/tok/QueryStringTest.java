...
public class QueryStringTest {
    ...
    @Test
    public void testIgnoredTokens() throws Exception {
        // First search without ignored tokens
        String queryWithout = """
                {
                  "query": {
                    "es_tok_query_string": {
        ...
        // Search with ignored tokens - ignore "文档"
        String queryWith = """
                {
                  "query": {
                    "es_tok_query_string": {
        ...
        // With ignored token "文档", we should get different (likely fewer or same)
        // results
        assertTrue("Results should change when ignoring tokens", hitsWithout >= 0 && hitsWith >= 0);
        System.out.println("Hits without ignored tokens: " + hitsWithout);
        System.out.println("Hits with ignored tokens ['文档']: " + hitsWith);
    }
    ...
    public void testCombinedFiltering() throws Exception {
        // Test with both ignored_tokens and max_freq
        String query = """
                {
                  "query": {
                    "es_tok_query_string": {
    ...
    @Test
    public void testEmptyIgnoredTokens() throws Exception {
        // Test with empty ignored tokens list
        String query = """
                {
                  "query": {
                    "es_tok_query_string": {
        ...
        String result = performSearch(query);
        assertTrue("Should work with empty ignored_tokens", result.contains("\"total\""));
    }
